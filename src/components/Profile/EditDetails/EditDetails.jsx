    ...
    return (
        <div className='p-2 lg:px-6 lg:py-6 rounded-sm my-4' style={{
            ...
            <div className='relative'>
                {
                    coverImage !== '' ? <img src={coverImage} alt="profile-image" className='w-[40px] h-[40px] lg:w-[80px] lg:h-[80px] rounded-full border-2 object-cover' /> : user?.image ? <img src={user?.image} alt="profile-image" className='w-[40px] h-[40px] lg:w-[80px] lg:h-[80px] rounded-full border-2 object-cover' /> : <CiUser className='w-[80px] h-[80px] rounded-full border-2' />
                }

                {/* <img src={cameraIcon} alt="avatar" className='w-[80px] h-[80px] rounded-full border-2' /> */}
                <div className='absolute bottom-1 left-12 cursor-pointer'>
                    {/* upload image */}
                    <img src={cameraIcon} alt="icon" className='' onClick={onCoverButtonClick} />
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-x-4 mt-4'>
                <TextField label='First Name' placeholder={user?.firstname} value={newFirstName} onChange={setNewFirstName} />
                <TextField label='Last Name' placeholder={user?.lastname} value={newLastName} onChange={setNewLastName} />
                ...
                <TextField label='Birth Date (day/month/year)' placeholder={user?.birthdate ? formatBirthdate(user.birthdate) : 'Format: 24/12/2002'} value={newBirthdate} onChange={setNewBirthdate} />
            </div>
            <div className='mt-4' >
                <button className='bg-[#1A985B] text-primary-text-color px-4 py-2 rounded-md' onClick={handleSaveChanges}>Save Changes</button>
            </div>
        </div>
    )
}
