...
const DeliveryInput = ({ label, type, placeholder, value, onChange, required }) => {
    ...
    return (
        <div className="relative my-2">
            <label
                htmlFor="name"
                className="-ms-1 rtl:right-3 rtl:left-full bg-white px-1 text-xs font-medium text-gray-900 mb-2"
            >
            ...
            {label === 'Country' ? (
                ...
                // />
                <Select
                    defaultValue={options.find(option => option.label === 'Saudi Arabia')}
                    className="mt-2"
                />
            ) : (
                <input
