...
const ShopSettings = () => {
    ...
    const handleShopSettings = async () => {
        ...
        if (shop) {
            ...
            if (shopNameAr) data.name = { ac: shopNameAr }
            if (phone) data.phone = phone
            if (categories) data.categories = categories
            if (descriptionEng) data.description = { en: descriptionEng }
            if (descriptionAr) data.description = { ac: descriptionAr }
            if (addressEng) data.region = { en: addressEng }
            if (addressAr) data.region = { ac: addressAr }
            ...
            if (refundPolicyAr) data.refund_policy = { ac: refundPolicyAr }
            if (returnAuthorizationMethodAr) data.return_authorization_method = { ac: returnAuthorizationMethodAr }
            if (returnShippingMethodAr) data.return_shipping_method = { ac: returnShippingMethodAr }
            ...
            try {
                ...
                showToast('Might be problem occured during shop creation | Make sure your internet connection is okay', 'err')
            }
        } else {
            // create shop
    ...
    const ImageUploadComponent = ({ img, label, reference }) => {
        return (
            <div className='flex items-center lg:items-start justify-center max-w-[290px] h-[150px] flex-col mt-4 relative'>
                ...
                <img src={img} alt="image" className='fluid object-cover w-full h-[110px] rounded-md' />
    ...
    return (
        <div className='font-main lg:-ms-10 px-2 lg:px-0'>
            <VendorHeader text='Shop Settings' />
            ...
            </div>
            {
                tab == 'english' ?
                    <div className='relative'>
                        ...
                        />
                        <InputField label='Shop Categories*' type='category' placeholder={categories ? categories : 'Provide information here'} value={categories} onChange={setCategories} />
                        ...
                        <InputField label='Shop Address*' type='text' placeholder={shop?.address?.en ? shop?.address?.en : 'Provide information here'} value={addressEng} onChange={setAddressEng} />

                        {/* save changes button */}
                        {/* <button
                            className='bg-[#319848] text-primary-text-color px-4 py-2 rounded-md'
                        ...
                        <InputField label='Refund Policy*' type='textField' placeholder='Provide information here' value={refundPolicyEng} onChange={setRefundPolicyEng} />
                        <InputField label='Return Authorization Method' type='textField' placeholder='Provide information here' value={returnAuthorizationMethodEng} onChange={setReturnAuthorizationMethodEng} />
                        <InputField label='Return Shipping Method' type='textField' placeholder='Provide information here' value={returnShippingMethodEng} onChange={setReturnShippingMethodEng} />

                        {/* save changes */}
                        <br />
                        <button
                            className='bg-[#319848] text-primary-text-color px-10 py-2 rounded-md'
                    ...
                    <div dir="rtl">
                        ...
                        <InputField label='اسم المحل*' type='text' placeholder='' value={shopNameAr} onChange={setShopNameAr} />
                        {/* <InputField label='Shop Phone*' type='number' placeholder={shop?.phone ? shop?.phone : 'Provide information here'} value={phone} onChange={setPhone} /> */}
                        <InputField label='وصف المتجر' type='textField' placeholder='' value={descriptionAr} onChange={setDescriptionAr} />
                        <InputField label='عنوان المحل' type='text' placeholder='' value={addressAr} onChange={setAddressAr} />

                        {/* save changes button */}
                        {/* <button className='bg-[#319848] text-primary-text-color px-4 py-2 rounded-md'>التنفيذ</button> */}
                        ...
                        <div className='relative mt-2'>
                            ...
                            <MdOutlineKeyboardArrowDown className='absolute top-7 right-2 w-5 h-5 text-gray-400' />
                            <label htmlFor="shipping_method" className='absolute top-[7px] left-2 rtl:right-3 rtl:left-full text-[12px] bg-white px-2 whitespace-nowrap'> طريقة الشحن * </label>
                        </div>
                        {/* return policy */}
                        <p className='font-[500] mt-8'>سياسات الإرجاع</p>
                        <InputField label='فترة العائد الربحي*' type='text' placeholder='' value={returnPeriod} onChange={setReturnPeriod} />
                        <InputField label='سياسة الاسترجاع*' type='textField' placeholder='' value={refundPolicyAr} onChange={setRefundPolicyAr} />
                        <InputField label='طريقة إذن العودة' type='textField' placeholder='' value={returnAuthorizationMethodAr} onChange={setReturnAuthorizationMethodAr} />
                        <InputField label='طريقة شحن الإرجاع' type='textField' placeholder='' value={returnShippingMethodAr} onChange={setReturnShippingMethodAr} />
                        {/* save changes */}
                        <br />
                        <button
                            className='bg-[#319848] text-primary-text-color px-10 py-2 rounded-md mt-4'
                            onClick={handleShopSettings}
                        >
                            {
                                shop ? 'Save Changes' : 'Create Shop'
