...
const VendorOrders = () => {
  ...
  const { isPending, isError, error, data } = useQuery({
    queryKey: ['orders', limit, page, selectedFilter],
    // start_date, end_date
    queryFn: async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_PATH}/order-items/get-all?shop_id=${user?.vendor_info?.shop?._id}&limit=${limit}&page=${page}${selectedFilter !== `All Date` ? `&start_date=${start_date}&end_date=${end_date}` : ''}`, {
          headers: {
        ...
        <div className='border-2 border-gray-100 rounded-md'>
          <div className="flex space-x-2 flex-wrap py-1 px-1">
            {['All Date', '12 Months', '30 Days', '7 Days', '24 Hour'].map((filter, index) => (
              <button
                key={index}
                className={`px-4 ${selectedFilter === filter ? 'bg-gray-200 py-1 rounded-md' : ''} `}
                variant="outline"
